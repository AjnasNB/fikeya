...
Fikeya Desktop starts in the Codex-style **Project UI**. Use the permanent view dropdown at the top of Chat to switch between **Project UI** and **Editor + Chat**. In Fikeya Desktop, `Ctrl+L` (`Cmd+L` on macOS) or the visible **Fikeya Chat** button toggles Chat in the right pane without hiding the editor. Compatible VS Code hosts keep their existing `Ctrl+L` behavior and use `Ctrl+Alt+I` or `Ctrl+Alt+L` (`Cmd` on macOS). Chat keeps the thread and bottom composer in focus. Compact controls beside the prompt select Agent, Plan, Research, or Multitask mode, choose the model, configure context, and open the Qarinah graph, usage receipts, or provider setup in temporary overlays. Code, Terminal, and Review continue to use the native workbench instead of being duplicated inside Chat.

Chat keeps a bounded conversation visible beside the editor. When workspace conversation history is enabled, a bounded snapshot is stored in VS Code workspace state so the latest thread can survive an extension-host restart; users can disable retention or clear the conversation from Chat. Chat text and changed-file paths pass through a best-effort known-pattern sanitizer, not comprehensive secret detection; attachment metadata is not secret-scanned and unknown formats may remain. Do not put secrets in saved Chat messages, attachment names, or file paths. Each terminal answer retains a validated metadata-only projection of up to 32 measured regular-file content-change entries (path, operation, existence, byte and line deltas, and before/after SHA-256), with separate disclosure for projection truncation and incomplete source accounting. These run-evidence attachments never enter provider history and never contain file bodies, tool output, or provider receipts. Conversation bodies are not written to analytics. Each submitted turn executes through the same reviewed Fikeya Runtime protocol, recompiles task-relevant Qarinah evidence when context is enabled and available, pauses for exact tool approvals, and attaches receipts to the latest run. The longer-lived project ledger belongs to Qarinah, while Fikeya Runtime stores only its documented session and receipt records. Reopening the command reveals the existing panel rather than creating duplicate sessions or message listeners. A webview message cannot invoke arbitrary workbench commands: every command and action is validated against a fixed allowlist.
...
**Multitask** runs selected advisory profiles in parallel against the same bounded prompt and workspace evidence. A workspace may retain up to 32 profiles, a run may select up to 16, and the local scheduler executes at most three by default. Each profile chooses its own configured provider and planner, researcher, or reviewer instruction. Tool approvals stay serialized and every terminal run keeps separate local provider evidence. When Qarinah capture succeeds, it adds a separate durable receipt; a failed capture is reported and may have appended partial deterministic ledger events. Shared-workspace write roles are intentionally excluded until isolated worktree execution is available.
...
If a provider reports HTTP 429, the extension can ask to retry the same task with another configured model, or remember an always-switch preference. When Qarinah context is enabled and available, each retry prepares task-relevant evidence again from the same workspace; Fikeya does not copy credentials or provider response bodies between profiles.

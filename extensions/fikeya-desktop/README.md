...
Fikeya Desktop opens the chat-first workspace at startup by default. Set `fikeya.chat.openAtStartup` to `false` to opt out. The setting is desktop-only and never forces Chat open in an ordinary VS Code extension host.

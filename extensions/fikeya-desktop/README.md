...
Use **Fikeya: Open Fikeya Chat** (`Ctrl+Alt+I` or `Ctrl+Alt+L`, `Cmd` on macOS) to open a conversation in the editor group beside the active code. Chat keeps the thread and composer in focus. The compact controls under the prompt select the model, open provider setup, choose Chat or Research, and open Plan, the Qarinah context graph, usage receipts, or model setup. Code, Terminal, and Review continue to use the native workbench instead of being duplicated inside Chat.

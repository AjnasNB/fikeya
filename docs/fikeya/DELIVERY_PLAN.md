...
The current runtime records a narrower provider receipt: request/response hashes and byte counts, latency, status, provider/model identity, and provider-reported input/output/cache tokens or `unavailable`. It does not yet claim the complete release evaluation above.

...
> **Current release status:** Fikeya `0.1.0-beta.3` is a source candidate and has not been published or Authenticode-signed. Treat every beta-3 Windows build as unsigned unless the final `release-verification.json` reports a valid trusted signature. An unsigned installer can show an unknown-publisher warning. This guide documents the approved signing paths; it does not claim that any current binary is signed.

...
Fikeya 0.1.0-beta.8 is the current public-beta source candidate. This document distinguishes:
...
Native browser actions are available only through typed, approval-gated tool calls. Playwright is the default. Puppeteer or Puppeteer Core requires an explicit engine selection and a dedicated reviewed package root; the transport never searches the active project for a package and never silently falls back. The root must declare exactly one supported package and retain an npm lock containing its exact version and integrity digest. Python verifies installed identity against that lock and binds the lock SHA-256 into the startup handshake. Every intercepted page request is authorized by the same Python URL guard, WebSockets and downloads are blocked, and the runtime bounds URLs, selectors, typed input, navigation and wait time, response text, bridge messages, pending guard requests, screenshots, action count, and session lifetime. It strips URL credentials, blocks unsafe schemes, treats page content as untrusted, excludes provider/API credentials from the child environment, and requires a separate explicit opt-in for private or loopback navigation. Browser cancellation closes the dedicated session and its complete Node/Chromium process tree; a non-wait action may acknowledge cancellation only after its already-bounded transport timeout returns.

Puppeteer and its transitive dependencies are trusted local executable code, not a sandbox. Package initialization happens before page interception and can use machine or network access granted by the operating system. Lock provenance prevents ambiguous selection and metadata drift but does not replace dependency review, `npm ci` integrity verification, or OS containment. Use a dedicated package root outside the workspace and an OS sandbox when the dependency itself is not trusted.
...
The beta.8 release workflow signs and verifies the outer Windows installer only. Nested executables, the VSIX, wheels, and source distributions are covered by hashes and release provenance, not claimed to carry independent Authenticode signatures. macOS, Linux, and ARM64 installers are not currently shipped.

...
The integrated runtime provides a dedicated browser session behind the same exact-call approval and cancellation boundaries as workspace and process tools. Playwright is the default transport. Puppeteer and Puppeteer Core are explicit optional transports selected through CLI or Desktop configuration; they do not silently load or fall back. Every Puppeteer navigation and subresource request round-trips through the same Python URL policy before Chromium may continue it. The session validates URLs, selectors, text inputs, timeouts, output sizes, screenshot paths, session limits, and redirects. Browser content is untrusted evidence. Public HTTPS navigation is the default; private or loopback targets require a separate one-run opt-in. Research can inspect and interact with web pages but cannot write a screenshot into the workspace.
...
Reviewed, disabled-by-default presets are available for separately installed Cockroach Browser and Cockroach Crawler executables. Enabling one records an exact manifest digest for one workspace but does not start the tool. Build and Research runs expose the preset's complete reviewed tool allowlist under namespaced broker tools. The integrated local-stdio host owns MCP framing, initialization, server identity/version checks, schema validation, bounded typed results, secret redaction, and full process-tree termination. Every call still crosses the existing exact one-use Agent Core approval and execution broker. Upstream network policy remains the external tool's responsibility, and the trusted local executable retains the desktop user's OS permissions unless a separate sandbox restricts it.
...
- **Fikeya Interop** implements standalone bounded local-stdio adapters for ACP agents, Codex app-server, and generic MCP tools. Runtime uses its own product host for the two reviewed preset integrations described above; ACP, Codex app-server, remote MCP, and arbitrary user-supplied MCP manifests remain outside that product path. Neither boundary is an OS sandbox.

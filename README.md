...
Fikeya 0.1.0-beta.6 is the current public-beta source candidate. It adds native `@` mentions for up to ten bounded workspace or computer files, preserves drafts and attachments until the runtime accepts a request, and fixes Project UI to Editor + Chat switching. It retains the durable audited Project lifecycle, five explicit agent intents (`Ask`, `Plan`, `Build`, `Review`, and `Research`), approval-gated browser verification, parallel read-only advisers, Qarinah context and graph inspection, and local usage and execution receipts.
...
Until beta.6 artifacts pass the release gates and are published, use the latest verified prerelease on the [GitHub Releases page](https://github.com/AjnasNB/fikeya/releases). Published releases include SHA-256 checksums, a machine-readable verification manifest, and GitHub artifact provenance. Treat beta.6 as unsigned unless its final `release-verification.json` reports a valid trusted signature; Windows can otherwise show an unknown-publisher warning. See the [release process](docs/fikeya/RELEASE.md) for the exact signing and promotion gates.

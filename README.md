...
- **Integrated native-agent core:** typed plan, act, observe, and review stages with cancellation, bounded retries, exact-call approvals, workspace tools, process-mutation hashes, test outcomes, and execution receipts in both Desktop and CLI.
...
Fikeya 0.1.0-beta.8 is the current public-beta source candidate. It prepares a newly opened folder before accepting its first request, keeps that request exactly once when setup fails, accepts native Explorer file drops, and gives Project UI the complete center surface without a closable editor tab. It retains native `@` mentions, the durable audited Project lifecycle, five explicit agent intents (`Ask`, `Plan`, `Build`, `Review`, and `Research`), approval-gated browser verification, parallel read-only advisers, Qarinah context and graph inspection, and local usage and execution receipts.

Beta.8 remains a prerelease. The release candidate embeds the reviewed Chromium payload only in the Windows x64 Desktop/VSIX runtime. A source or standalone CLI installation must install the `browser` extra and provision the pinned Playwright browser separately. Fikeya does not yet ship macOS, Linux, Windows ARM64, or macOS ARM64 installers. Stable promotion still requires trusted platform signing, clean-install verification on every supported package, and a verified Desktop update feed. The Windows release workflow signs and verifies the outer installer; it does not claim that every file nested inside the installer has an independent Authenticode signature.
...
Until beta.8 artifacts pass the release gates and are published, use the latest verified prerelease on the [GitHub Releases page](https://github.com/AjnasNB/fikeya/releases). Published releases include SHA-256 checksums, a machine-readable verification manifest, and GitHub artifact provenance. Treat beta.8 as unsigned unless its final `release-verification.json` reports a valid trusted signature; Windows can otherwise show an unknown-publisher warning. See the [release process](docs/fikeya/RELEASE.md) for the exact signing and promotion gates.

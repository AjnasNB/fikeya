...
Fikeya 0.1.0-beta.5 is the current public-beta source candidate. It adds the durable audited Project lifecycle, five explicit agent intents (`Ask`, `Plan`, `Build`, `Review`, and `Research`), approval-gated native browser verification, restart recovery, execution leases, cross-process cancellation, and evidence-bound completion. It retains Project Chat as the default Desktop surface, compact in-app BYOK model setup, one approval-gated coding lead with bounded parallel read-only advisers, attachment-safe background refreshes, Qarinah context and graph inspection, and local usage and execution receipts.

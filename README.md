...
Until beta.7 artifacts pass the release gates and are published, use the latest verified prerelease on the [GitHub Releases page](https://github.com/AjnasNB/fikeya/releases). Published releases include SHA-256 checksums, a machine-readable verification manifest, and GitHub artifact provenance. Treat beta.7 as unsigned unless its final `release-verification.json` reports a valid trusted signature; Windows can otherwise show an unknown-publisher warning. See the [release process](docs/fikeya/RELEASE.md) for the exact signing and promotion gates.

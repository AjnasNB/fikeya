...
`fikeya_runtime.mcp_stdio.McpStdioHost` is the process-owning host for an enabled preset. It rechecks the exact preset digest, delegates launch preparation and spawning to `ToolPresetLoader`, negotiates MCP initialization, checks the server package name and version against the reviewed SemVer range, validates the complete declared tool set, and exposes typed `tools/list` and `tools/call` results. JSON-RPC identifiers and schemas, request/response sizes, request counts, timeouts, session duration, stderr retention, and process shutdown are bounded. Resolved credentials are redacted from discovered definitions, typed results, remote errors, stderr, and diagnostics. A timeout, oversized line, protocol violation, failed initialization, or broker shutdown terminates the complete POSIX process group or Windows Job Object rather than only the direct child.
